	...
	private JsonUtil jsonUtil; // json解析
	  ...
	  @Resource //注解注入
	   private BacnetManager bacnetManager;
		...
		public WSocketClientEng(String uri,String clientId,String clientName,String eventName) throws URISyntaxException, InterruptedException{
            ...
            this.registerAdapter(clientId,clientName,AdapterConstants.WEBSOCKT_REGISTER);
		}
...
//        	   授权信息：: 0{"sid":"npZfRe1hcBnviupvAAAB","upgrades":[],"pingInterval":25000,"pingTimeout":60000}
        	   ...
        	   if("0".equals(preNum)){
        		   ...
        		   String eName = message.substring(message.indexOf("\"") + 1, message.indexOf(",")-1);
            		   ...
            		   try {
    					   ...
    					   if(AdapterConstants.WEBSOCKT_REGISTER.equals(callback)){
    						   if("true".equals((String)map.get("data"))){
    							   ...
    							   LOG.info("向websocketserver注册适配器失败：" + (String)map.get("message"));
    						   }
    					   }
    					   //向websocketserver推送数据后返回的信息
    					   else if(AdapterConstants.WEBSOCKT_CLIENTSEND.equals(callback)){
    						   if("true".equals((String)map.get("data"))){
    							   LOG.debug("向websocketserver推送数据成功！");
    						   }else{
    							   LOG.debug("向websocketserver推送数据失败：" + (String)map.get("message"));
    						   }
    					   }
    					   //websocketserver订阅接口
        ...
        public void onClose( int code, String reason, boolean remote ) {  
        	LOG.debug("关流--Connection closed by " + ( remote ? "remote peer" : "us" ) + "。code:" + code + ";reason:" + reason + ";remote:" + remote );
        	System.out.println( "关流--Connection closed by " + ( remote ? "remote peer" : "us" ) + "。code:" + code + ";reason:" + reason + ";remote:" + remote );  
        }  
         ...
         * @param adapterName
         */
        public void registerAdapter(String clientId,String clientName,String callback){
        	String regStr = "42[\"" + eventName + "\",{\"type\":\"register\",\"content\":" +
        			"{\"data\":{\"clientId\":\"" + clientId + "\",\"adapterName\":\"" + clientName + "\"},\"callback\":\"" + callback + "\"}}]";
...
//           	WebSocketUtils c = new WebSocketUtils( new URI( "ws://192.168.1.123:8000/socket.io/?transport=websocket" ), new Draft_17() );     
           	WSocketClientEng c = new WSocketClientEng( new URI( "ws://192.168.1.101:8090/socket.io/?transport=websocket" ), new Draft_17() );   
//           	WebSocketUtils c = new WebSocketUtils( new URI( "https://192.168.1.101:8000" ), new Draft_17() );  
           	
//           	c.connect();
   			System.out.println(c.connectBlocking());
   			c.send("42[\"client\",{\"server\":\"hello???\"}]");
...
//						"\"callback\": 21}";
        ...
        public boolean sendRtsubBak(String data,String message){
        	boolean bRst = false;
        	String str = "42[\"" + eventName + "\",{\"callback\":\"" + AdapterConstants.WEBSOCKT_RTSUB + "\",\"data\":\"" + data + "\"," +
        			"\"message\":\"" + message + "\"}]";
...
default:
	break;
        		...
        		pointId = this.getClientId() + "." + point.getControllerid() + "." + pointId; //适配器编号.控制器编号.点编号
    			sData += "{\"name\":\"" + pointId + "\",\"value\":\"" + point.getPointValue() + "\",\"ioproperty\":\"" + ioproperty+ "\"},";
        	}
        ...
        public boolean sendDataToWServer(String data){
        	...
        	String str = "42[\"" + eventName + "\",{" +
        			"\"type\":\"clientSend\"," +
        			"\"content\":{\"clientId\":\"" + this.clientId + "\",\"sourcetype\":\"adapter\",\"data\":" + data + "}" + 
        			"}]";
	    ...
	    public boolean reciWServerCont(Map<String,Object> map){
			...
			String str = "42[\"" + eventName + "\",{\"callback\":\"" + callback + "\",\"data\":\"" + bSend + "\"," +
        			"\"message\":\"\"}]";
...
//	    	0{"sid":"npZfRe1hcBnviupvAAAB","upgrades":[],"pingInterval":25000,"pingTimeout":60000}
	    ...
	    //	Map<String,Object> mapAuth = jsonUtil.jsonStrToMap(authorize.substring(1));
