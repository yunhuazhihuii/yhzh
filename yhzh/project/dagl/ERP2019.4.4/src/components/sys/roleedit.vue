...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getrole(){
              console.log('hahah')
              this.role = this.$route.params.rolecontent
              console.log(this.role)


              this.form.title = this.role[0].rolename
              this.form.desc = this.role[0].roledesc
              this.roleid = this.role[0].roleid

              this.checkedhomes = (this.role[0].homerole.substr(1)).substr(0,(this.role[0].homerole.substr(1)).length-1).replace(/"/g, "").split(',')

              this.checkedorders = (this.role[0].orderrole.substr(1)).substr(0,(this.role[0].orderrole.substr(1)).length-1).replace(/"/g, "").split(',')

              this.checkedcks = (this.role[0].ckrole.substr(1)).substr(0,(this.role[0].ckrole.substr(1)).length-1).replace(/"/g, "").split(',')

              this.checkedsys = (this.role[0].sysrole.substr(1)).substr(0,(this.role[0].sysrole.substr(1)).length-1).replace(/"/g, "").split(',')
