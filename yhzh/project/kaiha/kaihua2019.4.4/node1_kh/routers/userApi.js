...
router.post('/login', function (req, res) {
    nodeApi.query("SELECT * FROM YH_USER WHERE userenname=? and password=?",
...
router.post('/existeUser', function (req, res) {
    nodeApi.query("SELECT * FROM YH_USER WHERE userenname=?",[req.body.name],
      function(data){
        if(data.length>0){
           res.send({code: 1});
        }else{
...
router.post('/addAdminUser', function (req, res) {
  ...
  nodeApi.query('INSERT INTO YH_USER(name,password,admin) VALUES(?,?,?)',
...
router.post('/deleteUser', function (req, res) {
  ...
  nodeApi.query('DELETE FROM YH_USER WHERE name =?',[req.query.id,],
  function(){
      res.send({code: 1})
    });
});
...
router.post('/adminInfo', function (req, res) {
  nodeApi.query('update YH_USER SET name =?,password = ?,admin = ? WHERE id = ?',
...
router.post('/allUser', function (req, res) {
  nodeApi.query('select * from YH_USER',[],
  function(data){
      res.send({code: 1,data: data})
  });
});
