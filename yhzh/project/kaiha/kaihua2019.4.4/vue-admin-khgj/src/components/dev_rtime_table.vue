	...
	export default {
    	...
    	methods: {
			...
			*/
			qryAndSubsPoints(){
				var that = this;
				console.log("floor1与floor2 ==>",that.floor1 + ";" + that.floor2 + ";" + that.devtypeid);
				...
				getDevTable(that.devtypeid,that.floor1,that.floor2,res=>{
	            	...
	            	console.log("获取设备及点表信息==》",res.data);
		          	for (var i = 0; i < that.devPointData.length; i++) {
		          	 	for (var key in that.devPointData[i]){
		          	 		if(that.devPointData[i][key] && that.devPointData[i][key].pointId){
		          	 			this.in_outpoint.content.data.subscribeSubitem.push(that.devPointData[i][key].pointId);
		          	 		}
		          	 		// if(that.devPointData[i][key] && that.devPointData[i][key].pointId){
		          	 		// 	this.openAll.content.data.subscribeSubitem.push(that.devPointData[i][key].pointId);
		          	 		// }
		          	 	}
		          	}
	    	  		...
	    	  		console.log("123456");
	    	  		console.log("'clientToS', this.in_outpoint");
